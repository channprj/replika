  ...
  Plugin.prototype = {
    ...
    updateTime: function () {
      ...
      if(total_reading_time_remaining > 1) {
        text = total_reading_time_remaining + ' 분 남음';
      } else if(progress >= 1) {
        text = '고마워요 :-)';
      ...
      bubble.css('top', distance).text(text);//.fadeIn(20);
    ...
    calculate_total_time_words: function(post_content, element){
      ...
      post_content.each(function() {
        total += Math.round(60*$(element).text().split(' ').length/160); // 160 = number of words per minute
      });
